...
class plantacionController extends Controller
{
...

    {
        ...
        $result = ['message' => 'Plantacion creada correctamente', 'status' => 201];
    ...
    function show($id_usuario, $id_plantacion)
    {
        ...
        try {
            ...
            $result = ['message' => 'Plantacion no encontrada', 'status' => 500];
            return response()->json($result, 404);
        }

        if (!$resultQuery) {
            $result = ['message' => 'Plantacion no encontrada o no pertenece al usuario', 'status' => 404];
            return response()->json($result, 404);
        }

        $result = ['Plantacion' => $resultQuery, 'status' => 200];
        return response()->json($result, 200);
    }
    ...
    function delete($id)
    {
        ...
        try {
            ...
            $result = ['message' => 'Plantacion eliminada', 'status' => 200];
            return response()->json($result, 200);
        } catch (\Exception $e) {
    ...
    function update(Request $request, $id)
    {
        ...
        try {
            ...
            try {
                ...
                $result = ['message' => 'No se pudo actualizar la plantacion', 'status' => 500];
                return response()->json($result, 500);
            }
            $result = ['message' => 'Plantacion actualizada', 'status' => 200];
            return response()->json($result, 200);
        } catch (\Exception $e) {
